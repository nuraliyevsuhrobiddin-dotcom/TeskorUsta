...
export default function ListingCard({ listing, featured = false }: ListingCardProps) {
  return (
    <Link href={`/listings/${listing.slug}`} className="block interactive">
      <div 
        className={cn(
      ...
      >
        ...
        {/* Action Button */}
        <div className="mt-1">
          <button className="w-full flex items-center justify-center gap-2 bg-slate-50 dark:bg-slate-800/50 hover:bg-blue-50 dark:hover:bg-blue-500/10 text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 py-3 rounded-xl font-semibold text-sm transition-colors border border-slate-100 dark:border-slate-800">
